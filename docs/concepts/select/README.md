...
The behavior of the memoized selectors can be configured at a global level using the `selectorOptions` property in the options passed to the `NgxsModule.forRoot` call (see [Options](../store/options.md)).\
...
* **NOTE:** _The default for this setting will be changing to `false` in NGXS v4._\
